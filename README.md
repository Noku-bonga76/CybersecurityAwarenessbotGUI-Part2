...
4. Select the `CybersecurityChatbotGUI.csproj` file
5. Right-click `Greeting.wav` in Solution Explorer → **Properties** → set **Copy to Output Directory** to **Copy Always**
...
| two-factor | "Tell me about two-factor authentication" |
...
Commit Message 
1. Initial commit: Set up WinForms project structure and added all source files |
2. Added MainForm GUI with dark cyberpunk theme, ASCII logo header and voice greeting on launch |
3. Implemented ResponseEngine with keyword recognition for password, phishing, scam, privacy and malware |
4. Added SentimentDetector to identify worried, frustrated, curious and happy moods with empathetic responses |
5. Implemented UserMemory to store user name and favourite topic with delegate-based follow-up conversation flow |
6. Added input validation, error handling for unknown inputs and documented all classes with XML comments |
---

## Releases

| v2.0.0 | Part 2 - Initial GUI Release | WinForms GUI with ASCII logo, voice greeting, keyword recognition and dark theme |
